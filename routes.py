...
def calculate_sum():
    ...
    if view_type == 'student':
        ...
        total_payable = sum(order.total_price - order.paid_amount for order in orders)
...
@app.route('/validate_batch_delete', methods=['POST'])
@login_required
def validate_batch_delete():
    ...
    for order_id in order_ids:
        ...
        if order:
            has_class_records = len(order.class_records) > 0
            has_payment_records = len(order.payment_records) > 0
            has_salary_records = len(order.salary_records) > 0
            
            if has_class_records or has_payment_records or has_salary_records:
                orders_with_records.append({
                    'order_number': order.order_number,
                    'has_class_records': has_class_records,
                    'has_payment_records': has_payment_records,
                    'has_salary_records': has_salary_records
                })

    return jsonify({
        'has_records': len(orders_with_records) > 0,
        'orders_with_records': orders_with_records
    })

@app.route('/batch_delete_orders', methods=['POST'])
@login_required
